...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Prefect workflow tests for response_agent.py")
    parser.add_argument("--no-test-mode", action="store_true",
                        help="Run in normal mode instead of test mode")
    parser.add_argument("--debug", action="store_true",
                        help="Run in debug mode (raise exceptions)")
    args = parser.parse_args()

    success = run_workflow_tests(
        test_mode=not args.no_test_mode, debug=args.debug)
