...
def process_issue(
    ...
    try:
        ...
        if triggers.has_user_comment_on_pr(issue):
            repo_path = bot_tools.get_local_repo_path(repo_name)
            ...
            if branch_name:
                try:
                    ...
                    comments = get_issue_comments(issue)
                    user_comment = next((comment.body for comment in reversed(comments) 
                                      if "generated by blech_bot" not in comment.body), None)
                    ...
                    if user_comment:
                        ...
                        write_issue_response(issue, write_str + signature)
                    ...
                    # Clean up on error
                    back_to_master_branch(repo_path)
                    raise RuntimeError(f"Failed to process PR comment: {str(e)}")
