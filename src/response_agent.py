...
def generate_issue_response(
    ...
    if has_user_feedback(issue) or feedback_text:
        ...
        # Get the original bot response
        comments = get_issue_comments(issue)
        original_response = next(c.body for c in comments if "Generated by blech_bot" in c.body)
        
        # Get feedback text if not provided
        if not feedback_text:
            feedback_text = next(c.body for c in reversed(comments) 
                               if "Generated by blech_bot" not in c.body)
...
Reply "TERMINATE" in the end when everything is done."""
    ...
    # For non-feedback issues, run normal workflow
...
def has_user_feedback(issue: Issue) -> bool:
    """
    Check if there is user feedback after a bot response
    
    Args:
        issue: The GitHub issue to check
        
    Returns:
        True if there is a non-bot comment after a bot comment
    """
    comments = get_issue_comments(issue)
    
    # Track if we've seen a bot comment
    found_bot = False
    ...
    for comment in comments:
        if "Generated by blech_bot" in comment.body:
            ...
            # If we find a non-bot comment after a bot comment,
            # that's feedback
            return True
